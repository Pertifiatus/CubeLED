...
// Zeichnet nur die UNTERE Hälfte eines gefüllten Kreises — spaltenweise über
// das Standard-Midpoint-Kreisverfahren (nur Integer-Arithmetik, kein sin/cos),
// jede Spalte wird nur bis zur unteren Grenze statt über die volle Höhe gefüllt.
// Dadurch entsteht die Halbkreis-Form direkt beim Zeichnen, ohne vorher den
// vollen Kreis zu malen und die obere Hälfte wieder zu übermalen.
void fillHalfCircleBottom(int x0, int y0, int r, uint16_t color) {
  int16_t f     = 1 - r;
  int16_t ddF_x = 1;
  int16_t ddF_y = -2 * r;
  int16_t x     = 0;
  ...
  gfx->drawFastVLine(x0, y0, r + 1, color);  // Mittelspalte, untere Hälfte
  ...
  while (x < y) {
    ...
    gfx->drawFastVLine(x0 + x, y0, y + 1, color);
    gfx->drawFastVLine(x0 - x, y0, y + 1, color);
    gfx->drawFastVLine(x0 + y, y0, x + 1, color);
    gfx->drawFastVLine(x0 - y, y0, x + 1, color);
  }
}
...
// Menüname — bei allen vier Screens an derselben Stelle/Größe, füllt den
// oberen Freiraum deutlich mehr aus als die alte kleine Titelzeile.
...
// geleert (deckt auch den Sonderfall "nur Seite gewechselt" korrekt ab).
void drawTitle() {
  if (!needFullRedraw) return;
  drawCentered(SETTING_NAMES[curSetting], 100, 2, 0x8410);
}
...
// farbig. Ändert sich nur, wenn curSetting wechselt, und das passiert immer
// zusammen mit needFullRedraw.
void drawMenuDots() {
  if (!needFullRedraw) return;
...
void drawSaturationScreen() {
  ...
  if (needFullRedraw || needArcRedraw || satChanged || hueChanged) {
    fillHalfCircleBottom(CX, CY, 58, hsv8To565(hue, sat, 200));
  }
...
const float SIDE_SWEEP = (360.0f - NUM_SIDES * SIDE_GAP) / NUM_SIDES;
int prevActiveSide = -1;  // merkt sich den zuletzt aktiven Sektor für die Wheel-Reparatur

float sideSectorStart(int i) { return -90.0f + i * (SIDE_SWEEP + SIDE_GAP); }

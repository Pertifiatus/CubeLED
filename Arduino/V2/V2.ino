...
void fillArcSector(int cx, int cy, int innerR, int outerR,
  ...
  const float STEP = 1.0f * (float)M_PI / 180.0f;  // 1° für glatte Kanten
...
void repairHueRing(float markerDeg) {
  for (float d = markerDeg - 6.0f; d <= markerDeg + 6.0f; d += 1.0f) {
    float wrapped = fmodf(d + 360.0f, 360.0f);
...
void repairSatRing(float markerDeg, uint8_t hue) {
  for (float d = markerDeg - 6.0f; d <= markerDeg + 6.0f; d += 1.0f) {
    ...
    fillArcSector(CX, CY, 86, 108, d, d + 1.0f, hsv8To565(hue, s, 200));
  }
...
void drawSideDots() {
  bool colorChanged = (cfg[curSide].hue != cfgPrev[curSide].hue ||
                       cfg[curSide].sat != cfgPrev[curSide].sat);
  if (!needDotsRedraw && curSide == prevSide && !colorChanged) return;
...
void drawBrightnessScreen() {
  ...
  if (needFullRedraw || needArcRedraw ||
    ...
    // Prozentwert — korrektes Rect: size-3 = 24px hoch, ab y=CY-18=102 → bis y=126
    gfx->fillRect(CX - 50, CY - 20, 100, 30, BLACK);
    char buf[8];
...
void drawSideScreen() {
  ...
  // Aktive Seitennummer — korrektes Rect: size-4 = 32px hoch, ab y=CY-18=102 → bis y=134
  gfx->fillRect(CX - 30, CY - 20, 60, 38, BLACK);
  char buf[3];
